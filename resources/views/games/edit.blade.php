...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-semibold mb-8 text-center">Edit Game: {{ $game->name }}</h1>
    ...
    <form action="{{ route('games.update', $game->id) }}" method="POST" enctype="multipart/form-data" class="w-full max-w-2xl bg-white rounded-lg shadow-md p-6">
        ...
        <div class="mb-4">
            ...
            <input type="text" class="block w-full px-4 py-2 text-gray-700 bg-white border rounded-md focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:ring focus:ring-opacity-40" id="name" name="name" value="{{ $game->name }}" required>
        </div>
        <div class="mb-4">
            <label for="description" class="block mb-2 font-bold text-gray-700">Description</label>
            <textarea class="block w-full px-4 py-2 text-gray-700 bg-white border rounded-md focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:ring focus:ring-opacity-40" id="description" name="description" rows="4" required>{{ $game->description }}</textarea>
        </div>
        <div class="mb-4">
            <label for="developer" class="block mb-2 font-bold text-gray-700">Developer</label>
            <input type="text" class="block w-full px-4 py-2 text-gray-700 bg-white border rounded-md focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:ring focus:ring-opacity-40" id="developer" name="developer" value="{{ $game->developer }}" required>
        </div>
        <div class="mb-4">
            <label for="release_date" class="block mb-2 font-bold text-gray-700">Release Date</label>
            <input type="date" class="block w-full px-4 py-2 text-gray-700 bg-white border rounded-md focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:ring focus:ring-opacity-40" id="release_date" name="release_date" value="{{ \Carbon\Carbon::parse($game->release_date)->format('Y-m-d') }}" required>
        </div>      
        <div class="mb-4">
            <label for="genre" class="block mb-2 font-bold text-gray-700">Genre</label>
            <input type="text" class="block w-full px-4 py-2 text-gray-700 bg-white border rounded-md focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:ring focus:ring-opacity-40" id="genre" name="genre" value="{{ $game->genre }}" required>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            <label for="video" class="block mb-2 font-bold text-gray-700">Game Video (optional):</label>
            @if($game->video_path)
